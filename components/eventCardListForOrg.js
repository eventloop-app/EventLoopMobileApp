  ...
  return (
    <TouchableOpacity activeOpacity={0.7} onPress={onPress} style={[styles.shadowsButton]}>
      <View style={[styles.Container]}>
        {
          (item.endDate < moment().unix() * 1000) &&
          <View style={{
            position: 'absolute', flex: 1, width: '110%', height: '110%', backgroundColor: 'rgba(0,0,0,0.7)', justifyContent:'center', alignItems: 'center', zIndex: 50
          }}>
        ...
        <View style={{width: "100%", height: "100%", paddingHorizontal: 5, paddingVertical: 5}}>
          ...
          <View style={[styles.Location, {marginTop: 5}]}>
            <Ionicons name={item.type === 'ONSITE' ? 'ios-location-outline' : 'laptop-outline'} size={20}
          ...
          <View style={[styles.Location, {marginTop: 5}]}>
            ...
            <Text numberOfLines={1}
                  style={styles.TextLocation}>{`${item.numberOfRegister}/${item.numberOfPeople}`}</Text>
          </View>
        </View>
      </View>
    </TouchableOpacity>)
